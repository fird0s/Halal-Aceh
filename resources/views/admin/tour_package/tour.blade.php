        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th>Package Name</th>
              <th>Duration</th>
              <th style="text-align: center;">Price/Person</th>
              <th>Action</th>
            </tr>
          </thead>
          <tbody>
            @foreach ($tour_package as $data)
            <tr>
              <td><img width="40" src="{{{ asset('media/tour_package') }}}/{{ unserialize($data->images)[0] }}"></td>
              <td>{{ $data->package_name }}</td>
              <td>{{ $data->package_duration }}</td>
              <td style="text-align: center;">{{ $data->price_per_person }}</td>
              <td class="text-nowrap">
                <a href="{{ route('edit_tour_package', ['id' => $data->id]) }}" class="btn btn-sm btn-icon btn-flat btn-default" data-toggle="tooltip" data-original-title="Edit">
                  <i class="icon wb-wrench" aria-hidden="true"></i>
                </a>
                <a href="{{ route('delete_tour_package', ['id' => $data->id]) }}" class="btn btn-sm btn-icon btn-flat btn-default" data-toggle="tooltip" data-original-title="Delete">
